...
const scopes = 'User.Read.All Place.Read.All Application.ReadWrite.OwnedBy Application.ReadWrite.All Directory.ReadWrite.All Organization.ReadWrite.All AppRoleAssignment.ReadWrite.All Calendar.Read.All';
...
async function getGraphCode() {
    ...
    if (code || client_credentail_token) {
        ...
        const finalData = {
            servicePrincipalAppId: consentedServicePrincipal.appId,
...
async function getConsentedBarcoApplicationServicePrincipal(tokens: msGraphTokens): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        callUrlWithToken(
            new URL("https://graph.microsoft.com/v1.0/servicePrincipals?$filter=startswith(displayName, 'room-service')"),
            tokens.access_token,
            (data: any) => {
            console.log('*************************************************');
